/* $Id: main.c,v 1.1 1999/11/28 03:23:03 dougm Exp $ */
...
handle_parent_connection (int s)
{
    ...
    if (GTOP_S_VERBOSE)
	GTOP_S_LOG_MESSAGE (LOG_INFO, "Parent features = %lu",
			glibtop_server_features);

    if (GTOP_S_DEBUG)
	GTOP_S_LOG_MESSAGE (LOG_DEBUG, "SIZEOF: %u - %u - %u - %u - %u - %u",
	...
	if (cmnd->data_size) {
	    if (GTOP_S_DEBUG)
		GTOP_S_LOG_MESSAGE (LOG_DEBUG, "Client has %d bytes of data.",
				(int) cmnd->data_size);
	...
	case GLIBTOP_CMND_NETLOAD:
	    ...
	    break;
	default:
	    GTOP_S_LOG_MESSAGE (LOG_ERR, "Parent received unknown command %u.",
			    cmnd->command);
	    break;
	}
    }
